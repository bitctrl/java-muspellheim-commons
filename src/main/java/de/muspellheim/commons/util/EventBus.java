 ...
 *
 * <p>
 * Subscribe and publish events. Events are published in channels distinguished
 * by event type. Channels can be grouped using an event type hierarchy.
 *
 * <p>
 * You can use the default event bus instance {@link #getDefault}, which is a
 * singleton or you can create one or multiple instances of {@link EventBus}.
 *
 * <p>
 * Example:
 *
...
public class EventBus {
	 ...
	 *
	 * @param eventType  the event type, can be a super class of all events to
	 *                   subscribe
	...
	public <T> void subscribe(@NonNull Class<? extends T> eventType, @NonNull Consumer<T> subscriber) {
		List<Consumer<?>> s = subscribers.computeIfAbsent(eventType, _ -> new CopyOnWriteArrayList<>());
		s.add(subscriber);
	}
	...
	public <T> void unsubscribe(@NonNull Class<? extends T> eventType, @NonNull Consumer<T> subscriber) {
		subscribers.keySet().stream().filter(eventType::isAssignableFrom).map(subscribers::get)
	 ...
	 *
	 * <p>
	 * The event type is the class of <code>event</code>. The event is published to
	 * all consumers which subscribed to this event type or any super class.
	 *
		...
		public void run() {
			while (!Thread.currentThread().isInterrupted()) {
				try {
					...
					subscribers.keySet().stream().filter(type -> type.isAssignableFrom(eventType))
