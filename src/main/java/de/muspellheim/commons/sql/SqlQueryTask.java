...
public class SqlQueryTask<T> implements RunnableFuture<T> {
	 ...
	 */
	public SqlQueryTask(@NonNull DataSource dataSource, @NonNull String sql, @NonNull SqlQuery<T> query) {
	...
	public synchronized void run() {
		...
		try (Connection connection = dataSource.getConnection()) {
			try (PreparedStatement localStatement = connection.prepareStatement(sql)) {
				this.statement = localStatement;
				result = query.execute(localStatement);
				state = NORMAL;
			}
		} catch (Exception e) {
			if (state != CANCELLED) {
	...
	public T get(long timeout, @NonNull TimeUnit unit)
		...
		if (state == NEW && !barrier.await(timeout, unit)) {
